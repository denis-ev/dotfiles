...
if [ "$1" = "copy" ]; then
  ...
  echo -n "$REMOTE_PART" | pbcopy
fi

echo "$REMOTE_PART | $(topcolour $SPEED_WARNING $REMOTE_WARNING) font=Courier"
...
echo "Copy WAN IP | terminal=false bash='$0' param1=copy"
...
echo "Terminal: Reset ARP| bash='sudo arp -a -d'"
